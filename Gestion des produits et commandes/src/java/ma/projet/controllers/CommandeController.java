    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("op") != null) {
            ...
            if (op.equals("update")) {
                ...
                c.setClient(ps.getById(clientId));
                c.setCode(code);
...
//                pps.update(produi);
            ...
            if (op.equals("afficherProduitsDeCommande")) {
                ...
                for (LigneCommandeProduit produit : produits) {
                    ...
                    out.println("<td><button type='button' style='background-color: red; color: white; padding: 10px 20px; font-size: 24px; cursor: pointer;' onclick='supprimerProduitDeCommande(" + produit.getProduit().getId() + ", " + produit.getCommande().getId() + ", " + produit.getQuantite() + ")'>Supprimer</button></td>");
            ...
            String code = request.getParameter("code");
            int clientId = Integer.parseInt(request.getParameter("client"));
            cs.create(new Commande(date, code, ps.getById(clientId)));
            response.sendRedirect("Commande.jsp");
